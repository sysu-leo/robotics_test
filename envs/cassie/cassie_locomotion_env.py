...
from envs.cassie import robot

from .gen_xml import builder


class CassieLocomotionEnv(mujoco_env.MujocoEnv):
    def __init__(self, model_path = None):
        ...
        # set up interface
        self.interface = robot_interface.RobotInterface(self.model, self.data, 'left-hip-roll', 'right-hip-roll')
        # set up task
        self.task = task.Task(client=self.interface,
                                             dt=control_dt,
        ...
        self.robot = robot.CassieRobot(pdgains.T, control_dt, self.actuators, self.interface)

        # define indices for action and obs mirror fns
        base_mir_obs = [0.1, -1, 2, -3,              # root orient
                        -4, 5, -6,                   # root ang vel
                        13, -14, -15, 16, -17,   # motor pos [1]
                         7,  -8,  -9, 10, -11,   # motor pos [2]
                        25, -26, -27, 28, -29,   # motor vel [1]
                        19, -20, -21, 22, -23,   # motor vel [2]
        ]
        ...
        self.robot.mirrored_acts = [6, -7, -8, 9, -10,
                                    0.1, -1, -2, 3, -4,]
