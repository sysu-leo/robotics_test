...
def _calc_fwd_vel_reward(self):
    ...
    error = np.linalg.norm(root_vel - self.command_x_vel)
    return np.exp(-error)

def _calc_letaral_vel_reward(self):
    # forward vel reward
    root_vel = self._client.get_qvel()[1]
    error = np.linalg.norm(root_vel - self.command_y_vel)
    return np.exp(-error)

def _calc_yaw_vel_reward(self):
    # forward vel reward
    root_vel = self._client.get_qvel()[3]
    error = np.linalg.norm(root_vel - self.command_yaw_vel)
    return np.exp(-error)
